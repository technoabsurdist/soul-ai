...
app.post("/entry", async (req, res) => {
  ...
  const result = await pool.query(
    "INSERT INTO documents (text, title, user_id) VALUES ($1, $2, $3) RETURNING *",
    [text, title, userId]
  )
  res.send(result.rows[0])
})
...
app.post("/chat", async (req, res) => {
  ...
  const journalResults = await pool.query(
    "SELECT text FROM documents WHERE user_id = $1",
    [userId]
  )
  ...
  const response = await helpers.modelResponse(
    name.rows[0].name,
    text,
    journalText
  )
...
app.get("/chat/history", async (req, res) => {
  ...
  const formattedHistory = result.rows
    .map(row => [
